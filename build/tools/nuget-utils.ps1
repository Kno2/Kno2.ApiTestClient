task Package-Restore {
	...
	Download-Nuget $nugetpath
	Package-Restore $nugetpath $solution_file $nuget_config_path
}

function Package-Restore {
	param(
		[string]$nugetpath = $(throw "nuget path is required"),
		[string]$solution_file = $(throw "solution file is required"),
		[string]$nuget_config_path
	)
	if($nuget_config_path){
		Write-Host "Using config file $nuget_config_path"
		& $nugetpath restore $solution_file -ConfigFile $nuget_config_path -NOCACHE
	} else {
...
function Download-Nuget {
	...
	if ((Test-Path -path $nugetpath) -eq $False){
		Write-Host "Downloading to $nugetpath"
		Get-WebFile "https://nuget.org/nuget.exe" $nugetpath
	}
	else {
		Write-Host "Checking for $nugetpath updates"
